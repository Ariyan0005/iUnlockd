...
import { Search, Server, Clock, ArrowRight } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-6">
      ...
      {loading && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Loading services">
          {[0, 1, 2].map((item) => <div key={item} className="h-44 animate-pulse rounded-xl border border-border bg-muted/60" />)}
        </div>
      )}
      {error && (
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p>No server services found{search ? ` for "${search}"` : ""}.</p>
        </div>
      )}
            ...
            {filtered
              ...
              .map((service) => (
                <ServiceCard
                  ...
                  service={service}
                  onSelect={() => navigate(`/services/${getServiceSlug(service)}`)}
                />
              ))}
          </div>
        </div>
